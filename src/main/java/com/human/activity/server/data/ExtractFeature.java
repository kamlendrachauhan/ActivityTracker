...
/**
 * We use labeled accelerometer data from users thanks to a device in their
 * pocket during different activities (walking, sitting, jogging, ascending
 * stairs, descending stairs, and standing).
 *
 * The accelerometer measures acceleration in all three spatial dimensions as
 * following:
 *
 * - Z-axis captures the forward movement of the leg - Y-axis captures the
 * upward and downward movement of the leg - X-axis captures the horizontal
 * movement of the leg
 *
 * After several tests with different features combination, the ones that I have
 * chosen are described below:
 *
 * - Average acceleration (for each axis) - Variance (for each axis) - Average
 * absolute difference (for each axis) - Average resultant acceleration: 1/n * ∑
 * √(x² + y² + z²) - Average time between peaks (max) (for each axis)
 *
 */
public class ExtractFeature {
	...
	public static double[] computeAvgAbsDifference(JavaRDD<double[]> data, double[] mean) {
		...
		JavaRDD<Vector> abs = data.map(record -> new double[] { Math.abs(record[0] - mean[0]),
				Math.abs(record[1] - mean[1]), Math.abs(record[2] - mean[2]) }).map(Vectors::dense);
	...
	public static double computeResultantAcc(JavaRDD<double[]> data) {
		...
		JavaRDD<Vector> squared = data
				.map(record -> Math.pow(record[0], 2) + Math.pow(record[1], 2) + Math.pow(record[2], 2)).map(Math::sqrt)
	...
	public Double computeAvgTimeBetweenPeak(JavaRDD<long[]> data) {
		...
		// keep the timestamp of data point for which the value is greater than
		// 0.9 * max
		// and sort it !
		JavaRDD<Long> filtered_y = data.filter(record -> record[1] > 0.9 * max[1]).map(record -> record[0])
		...
		if (filtered_y.count() > 1) {
			...
			JavaRDD<Vector> product = firstRDD.zip(secondRDD).map(pair -> pair._1() - pair._2())
					// and keep it if the delta is != 0
					.filter(value -> value > 0).map(line -> Vectors.dense(line));
