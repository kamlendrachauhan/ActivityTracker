...
import com.human.activity.server.data.PrepareData;
import com.human.activity.server.model.DecisionTrees;
import com.human.activity.server.model.RandomForests;
...
import static com.human.activity.server.data.ExtractFeature.*;
...
public class RecognizeActivity {
  ...
  public static void main() {
   ...
   CassandraJavaRDD<CassandraRow> cassandraRowsRDD = javaFunctions(sc).cassandraTable("activityrecognition", "trainingacceleration");
    ...
    for (int i = 1; i < 38; i++) {
      ...
      for (String activity: ACTIVITIES) {
        ...
        JavaRDD<Long> times = cassandraRowsRDD.select("timestamp")
                                              .where("userID=? AND activity=?", i, activity)
                                              .withAscOrder()
        ...
        if (100 < times.count()) {
          ...
          for (Long[] interval: intervals) {
            for (int j = 0; j < interval[2]; j++) {
              ...
              JavaRDD<CassandraRow> data = cassandraRowsRDD.select("timestamp", Constants.ACC_X, Constants.ACC_Y, Constants.ACC_Z)
                  .where("userID=? AND activity=? AND timestamp < ? AND timestamp > ?", i, activity, interval[1] + j * 5000000000L, interval[1] + (j - 1) * 5000000000L)
                  .withAscOrder().cache();
